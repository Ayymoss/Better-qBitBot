    ...
    ILogger<MessageCreateHandler> logger) : IMessageCreateGatewayHandler
{
    ...
    private async Task HandleInvocationOnBehalf(Message message, RestMessage targetMessage)
    {
        ...
        // Gather context from the target user's messages only
        var (context, attachments) = await GatherUserContext(message, targetMessage.Author.Id);
    ...
    private async Task<(string Context, List<AttachmentInfo> Attachments)> GatherUserContext(Message invokingMessage, ulong contextUserId)
    {
        ...
        var userMessages = recentMessages
            ...
            .Where(m => m.Id != invokingMessage.Id)
